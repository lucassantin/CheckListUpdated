...
import { generateProgressoPercentage } from "../utils/generate-progressopercentage"
...
export function HabitDay({amountOfHabits = 0, amountCompleted = 0, data, ...rest }: Props) {

  const amountAccomplishedPercentage = amountOfHabits > 0 ? generateProgressoPercentage(amountOfHabits, amountCompleted) : 0;
  const today = dayjs().startOf('day').toDate();
  const isCurrentToday = dayjs(data).isSame(today);
